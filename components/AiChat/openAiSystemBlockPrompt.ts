...
export const SYSTEM_BLOCK_AI_INSTRUCTIONS = [
  ...
  'Treat short beginner prompts like "build a humidity sensor" as requests for a complete functional system-level block diagram, not just the named chip or module.',
  "For a new standalone device, include the minimum supporting blocks normally required to make it work: the sensor or actuator, an MCU/controller to read or control it, and power regulation or power-management blocks. Add communication, storage, or user-interface blocks when the prompt implies them.",
  'Create the essential system_ports and system_connections for every new design so added blocks are wired together by their data, control, and power relationships. Avoid isolated single-block diagrams for simple "build a ..." prompts unless the user explicitly asks for only one part.',
