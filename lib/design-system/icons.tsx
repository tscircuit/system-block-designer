import type { CSSProperties } from "react"

const ICON_PATHS: Record<string, string> = {
  chip: '<rect x="6" y="6" width="12" height="12" rx="1.5"/><rect x="9.5" y="9.5" width="5" height="5" rx=".6"/><path d="M9 6V3M15 6V3M9 21v-3M15 21v-3M6 9H3M6 15H3M21 9h-3M21 15h-3"/>',
  antenna:
    '<circle cx="12" cy="9" r="2"/><path d="M12 11v9M7.5 4.5a6.4 6.4 0 0 1 9 0M5 7a9.9 9.9 0 0 1 14 0"/>',
  buffer: '<path d="M7 5 18 12 7 19z"/><path d="M3 9v6M21 12h-3"/>',
  memory:
    '<rect x="5" y="6" width="14" height="12" rx="1.5"/><path d="M8 6V3.5M12 6V3.5M16 6V3.5M8 18v2.5M12 18v2.5M16 18v2.5M9 10h6M9 13h6"/>',
  key: '<circle cx="8" cy="12" r="4"/><path d="M11 11h9l-1.5 2M16 11v3"/>',
  power:
    '<rect x="5" y="7" width="14" height="10" rx="2"/><path d="M9 7V4M15 7V4M10 12l1.6 0M14 10v4"/><circle cx="11" cy="12" r=".4"/>',
  nfc: '<rect x="6" y="6" width="12" height="12" rx="2"/><path d="M9 15V9l6 6V9"/>',
  battery:
    '<rect x="3" y="8" width="14" height="9" rx="1.5"/><path d="M17 11h2v3h-2M6 12.5h3M7.5 11v3"/>',
  battery2:
    '<rect x="3" y="9" width="13" height="7" rx="1.5"/><path d="M16 11h2v3h-2"/><path d="M6.5 12.5h2.5M9 11.2v2.6"/><path d="M11 8.5l1.5-2 1.2 2"/>',
  header:
    '<rect x="4" y="9" width="16" height="6" rx="1"/><path d="M7 9V6M11 9V6M15 9V6"/>',
  monitor:
    '<rect x="4" y="6" width="16" height="10" rx="1.5"/><path d="M6 13l2-3 2 2 2-4 2 5M9 19h6M12 16v3"/>',
  balance:
    '<rect x="5" y="9" width="12" height="7" rx="1.5"/><path d="M17 11h2v3h-2"/><path d="M9 12h4M11 10v4"/>',
  uwb: '<circle cx="12" cy="12" r="2"/><path d="M8.5 8.5a5 5 0 0 0 0 7M15.5 8.5a5 5 0 0 1 0 7M6 6a9 9 0 0 0 0 12M18 6a9 9 0 0 1 0 12"/>',
  shifter: '<path d="M5 6 12 11 5 16z"/><path d="M19 8 12 13 19 18z"/>',
}

interface IconProps {
  name: string
  size?: number
  x?: number
  y?: number
  style?: CSSProperties
}

export function Icon({ name, size = 24 }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth={1.7}
      strokeLinecap="round"
      strokeLinejoin="round"
      dangerouslySetInnerHTML={{ __html: ICON_PATHS[name] ?? ICON_PATHS.chip }}
    />
  )
}

export function BlockIcon({ name, x = 0, y = 0, size = 24, style }: IconProps) {
  return (
    <svg
      x={x}
      y={y}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth={1.7}
      strokeLinecap="round"
      strokeLinejoin="round"
      style={{ pointerEvents: "none", ...style }}
      dangerouslySetInnerHTML={{ __html: ICON_PATHS[name] ?? ICON_PATHS.chip }}
    />
  )
}
